...
//! ----------------------------------------------------------------------------
export const toggleInspirationSalonAction = async () => {
  try {
    const headers = await getAuthHeaders();

    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACK_URL}/users/inspiration-salon`,
      {
        method: "PATCH",
        headers,
      },
    ...
    console.error(
      "Erreur lors de la mise à jour du statut inspiration du salon :",
      error,
    );
    throw error;
  }
...
//! RECUPERER LE STATUT D'INSPIRATION DU SALON

//! ----------------------------------------------------------------------------
export const fetchInspirationSalonStatusAction = async (salonId: string) => {
  try {
    const headers = await getAuthHeaders();
    ...
    const isInspirationSalon =
      data?.isInspirationSalon ?? data?.user?.isInspirationSalon ?? false;
