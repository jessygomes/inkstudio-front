...
export default function MessageList() {
  ...
  const fetchConversations = useCallback(
    async (page = 1, status?: ConversationStatus) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const handleRetry = () => fetchConversations(1, statusFilter);
  const handlePageChange = (page: number) => fetchConversations(page, statusFilter);
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => {
      const otherUser =
        conversation.salonId === session?.user?.id ? conversation.client : conversation.salon;
  ...
  return (
    <section className="w-full space-y-4 pb-20 lg:pb-8">
      <PageHeader
        icon={<MessageCircle size={17} className="text-tertiary-400" />}
        title="Messagerie"
      />
      ...
      {!isFreeAccount && (
        <>
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-3">
            ...
            />
            <SummaryCard
              icon={isActiveStatus ? <MessageCircle size={17} /> : <Archive size={17} />}
              label="Vue actuelle"
              value={isActiveStatus ? "Actives" : "Archivées"}
            ...
            search={
              <label className="relative block w-full">
                ...
                <input
                  ...
                  placeholder="Rechercher un client ou un sujet…"
                  className="h-10 w-full py-2 pl-9 pr-3 text-sm text-white outline-none placeholder:text-white/30 font-one"
                />
              </label>
            }
            filters={
                ...
                {FILTERS.map((status) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-lg px-3 text-xs font-medium transition font-one ${
                        isSelected
                          ? "bg-tertiary-400/15 text-tertiary-400"
                          : "text-white/45 hover:bg-white/[0.05] hover:text-white/75"
                      }`}
                    >
                      {status === "ACTIVE" ? <MessageCircle size={13} /> : <Archive size={13} />}
                      {status === "ACTIVE" ? "Actives" : "Archivées"}
                    </button>
                  );
                })}
              </div>
      ...
      {isFreeAccount ? (
        <LockedFeatureCard
          ...
          description="Débloquez les notifications en temps réel, la messagerie interne illimitée et les pièces jointes."
          features={["Messagerie interne illimitée", "Pièces jointes", "Réponses automatiques"]}
        ...
        <div className="overflow-hidden rounded-[24px] border border-white/[0.08] bg-gradient-to-br from-white/[0.045] to-white/[0.015] shadow-xl shadow-black/15">
          <header className="flex items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-3.5 sm:px-5">
            <div>
              <h2 className="text-sm font-semibold text-white font-one">
                {isActiveStatus ? "Boîte de réception" : "Conversations archivées"}
              </h2>
              <p className="mt-0.5 text-[11px] text-white/40 font-two">
                {isActiveStatus
                  ? "Vos échanges clients les plus récents"
                  : "Les échanges que vous avez classés"}
              </p>
            </div>

            {!loading && totalPages > 1 && (
              <span className="rounded-full border border-white/10 bg-black/15 px-2.5 py-1 text-[10px] text-white/45 font-one">
          ...
          <div className="p-2 sm:p-3">
            ...
            {error && (
              <EmptyState
                ...
                tone="error"
              >
                <DashboardButton onClick={handleRetry} className="mt-4 !min-w-0">
                  <RefreshCw size={14} />
                  Réessayer
                </DashboardButton>
              </EmptyState>
            ...
            {!loading && !error && conversations.length === 0 && (
              <EmptyState
                icon={isActiveStatus ? <Inbox size={25} /> : <Archive size={25} />}
                title={isActiveStatus ? "Votre boîte de réception est vide" : "Aucune conversation archivée"}
                description={
                  isActiveStatus
                    ? "Les nouveaux échanges avec vos clients apparaîtront ici."
            ...
            {!loading && !error && conversations.length > 0 && filteredConversations.length === 0 && (
              <EmptyState
                ...
                description={`Aucune conversation ne correspond à « ${search} ».`}
              />
            )}
              ...
              <div className="hidden items-center gap-1 sm:flex">
                {pageNumbers.map((page) => (
                  <button
                    ...
                    className={`h-8 min-w-8 cursor-pointer rounded-lg px-2 text-xs transition font-one ${
                      currentPage === page
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-2xl border p-3 ${
        accent ? "border-tertiary-400/25 bg-tertiary-400/[0.08]" : "border-white/[0.08] bg-white/[0.025]"
      } ${className}`}
    >
      <span
        className={`flex h-9 w-9 items-center justify-center rounded-xl border ${
          accent
            ? "border-tertiary-400/25 bg-tertiary-400/10 text-tertiary-400"
      ...
      <div>
        <p className="text-[10px] uppercase tracking-[0.1em] text-white/35 font-one">{label}</p>
        <p className={`mt-0.5 text-sm font-semibold font-one ${accent ? "text-tertiary-400" : "text-white"}`}>
          {value}
        </p>
      </div>
    </div>
      ...
      <span
        className={`mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border ${
          tone === "error"
            ...
            : "border-tertiary-400/20 bg-tertiary-400/[0.08] text-tertiary-400"
        }`}
      >
        {icon}
      </span>

      <h3 className="text-base font-semibold text-white font-one">{title}</h3>
      <p
        className={`mt-1.5 max-w-md text-xs leading-relaxed font-two ${
          tone === "error" ? "text-red-200/70" : "text-white/40"
        }`}
      >
        {description}
      </p>
